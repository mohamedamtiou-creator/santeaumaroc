...
import { DeleteBtn } from "./_components/GlossaryActions";
...
export const metadata: Metadata = { title: "Glossaire — Admin SantéauMaroc" };

type SortKey = "terme" | "statut" | "date";
const SORTS: Record<SortKey, (dir: SortDir) => Prisma.GlossaryTermOrderByWithRelationInput[]> = {
  terme:  (dir) => [{ term: dir }],
  statut: (dir) => [{ status: dir }, { updatedAt: "desc" }],
  date:   (dir) => [{ updatedAt: dir }],
};
const DEFAULT_ORDER: Prisma.GlossaryTermOrderByWithRelationInput[] = [{ updatedAt: "desc" }];
...
export default async function AdminGlossairePage({
  ...
  const [terms, totalCount] = await Promise.all([
    prisma.glossaryTerm.findMany({
      ...
      take,
      select: {
        id: true, term: true, slug: true, category: true, status: true,
  ...
  const buildUrl = buildPageUrl("/admin/glossaire", { tri, dir });

  const newBtn = (
    <Link
      href="/admin/glossaire/nouveau"
    ...
    <div>
      <div className="mb-6">
        <AdminPageHeader
          ...
          subtitle={`${totalCount} terme${totalCount !== 1 ? "s" : ""} au total`}
          actions={newBtn}
        />
      </div>
      ...
      {terms.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          <p className="text-slate-500 mb-4">Aucun terme pour l’instant.</p>
          <Link href="/admin/glossaire/nouveau" className="text-primary-600 underline text-sm">Créer le premier terme</Link>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/80">
                <SortableTh col="terme" label="Terme" basePath="/admin/glossaire" tri={tri} dir={dir}
                ...
                <SortableTh col="statut" label="Statut" basePath="/admin/glossaire" tri={tri} dir={dir}
                  className="text-start px-4 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide hidden sm:table-cell" />
                <th className="text-start px-4 py-3 font-semibold text-slate-600 text-xs uppercase tracking-wide hidden lg:table-cell">Relecture</th>
                <SortableTh col="date" label="Date" basePath="/admin/glossaire" tri={tri} dir={dir}
                ...
                return (
                  <tr key={t.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-5 py-4">
                      ...
                      <p className="text-xs text-slate-500 font-mono mt-0.5">/glossaire/{t.slug}</p>
                    </td>
                    <td className="px-4 py-4 hidden md:table-cell">
                      <span className="text-xs text-slate-600">{CATEGORY_LABEL[t.category] ?? t.category}</span>
                    </td>
                    ...
                    <td className="px-4 py-4 hidden lg:table-cell">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${t.reviewedAt ? "bg-secondary-50 border-secondary-200 text-secondary-700" : "bg-slate-50 border-slate-200 text-slate-400"}`}>FR</span>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${t.arReviewedAt ? "bg-secondary-50 border-secondary-200 text-secondary-700" : "bg-slate-50 border-slate-200 text-slate-400"}`}>AR</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 hidden lg:table-cell text-xs text-slate-500">{fmtDate(t.updatedAt)}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-end gap-1">
                        {t.status === "PUBLISHED" && t.reviewedAt && (
                          <a
                            href={`/glossaire/${t.slug}`} target="_blank" rel="noopener noreferrer"
                        ...
                        <Link
                          href={`/admin/glossaire/${t.id}/modifier`}
          ...
          <AdminListFooter page={page} total={totalCount} buildUrl={buildUrl} noun="termes" />
        </div>
      )}
    </div>
  );
