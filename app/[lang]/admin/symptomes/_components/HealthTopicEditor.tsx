...
import { createTopic, updateTopic } from "@/features/health-topic/actions";
...
/** Ré-indente un JSON pour l'affichage, sinon rend la valeur brute. */
...
export function HealthTopicEditor({ specialties, topic }: { specialties: SpecialtyOption[]; topic?: TopicData }) {
  const isEdit = !!topic;
  const [pending, start] = useTransition();

  const [kind,          setKind]          = useState(topic?.kind          ?? "SYMPTOM");
  const [term,          setTerm]          = useState(topic?.term          ?? "");
  const [slug,          setSlug]          = useState(topic?.slug          ?? "");
  const [shortAnswer,   setShortAnswer]   = useState(topic?.shortAnswer   ?? "");
  const [causes,        setCauses]        = useState(topic?.causes        ?? "");
  const [redFlags,      setRedFlags]      = useState(topic?.redFlags      ?? "");
  const [whenToConsult, setWhenToConsult] = useState(topic?.whenToConsult ?? "");
  const [faqJson,       setFaqJson]       = useState(() => pretty(topic?.faqJson ?? null));
  const [synonyms,      setSynonyms]      = useState((topic?.synonyms      ?? []).join(", "));
  const [specialtyId,   setSpecialtyId]   = useState(topic?.specialtyId   ?? "");
  const [relatedSlugs,  setRelatedSlugs]  = useState((topic?.relatedSlugs  ?? []).join(", "));
  const [glossarySlugs, setGlossarySlugs] = useState((topic?.glossarySlugs ?? []).join(", "));
  const [sources,       setSources]       = useState(() => pretty(topic?.sources ?? null));

  const [termAr,          setTermAr]          = useState(topic?.termAr          ?? "");
  const [shortAnswerAr,   setShortAnswerAr]   = useState(topic?.shortAnswerAr   ?? "");
  const [causesAr,        setCausesAr]        = useState(topic?.causesAr        ?? "");
  const [redFlagsAr,      setRedFlagsAr]      = useState(topic?.redFlagsAr      ?? "");
  const [whenToConsultAr, setWhenToConsultAr] = useState(topic?.whenToConsultAr ?? "");
  const [faqJsonAr,       setFaqJsonAr]       = useState(() => pretty(topic?.faqJsonAr ?? null));
  const [sourcesAr,       setSourcesAr]       = useState(() => pretty(topic?.sourcesAr ?? null));

  const [status,          setStatus]          = useState(topic?.status ?? "PUBLISHED");
  ...
  function handleSave(publish: boolean) {
    setError("");
    if (!term.trim())        { setError("Le terme (FR) est obligatoire.");        return; }
    ...
    start(async () => {
      try {
        if (isEdit) await updateTopic(topic.id, fd);
        else await createTopic(fd);
      } catch (e) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
        ...
        <div>
          <label className={labelCls}>Terme * <span className="normal-case font-normal text-slate-500">(ex. « Mal de tête »)</span></label>
          <input type="text" value={term} onChange={(e) => handleTermChange(e.target.value)}
            placeholder="Libellé du symptôme" className={inputCls} />
        </div>

        <div>
          <label className={labelCls}>Slug URL</label>
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 whitespace-nowrap font-mono">/symptomes/</span>
            <input type="text" value={slug} onChange={(e) => setSlug(e.target.value)}
              placeholder="mal-de-tete" className={inputCls} />
          </div>
        </div>
        ...
        <div>
          ...
          <input type="text" value={synonyms} onChange={(e) => setSynonyms(e.target.value)}
            placeholder="céphalée, migraine" className={inputCls} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
        ...
        <div>
          <label className={labelCls}>Réponse courte * <span className="normal-case font-normal text-slate-500">(40-70 mots, extractible par les IA)</span></label>
          <textarea value={shortAnswer} onChange={(e) => setShortAnswer(e.target.value)} rows={3}
            placeholder="Réponse concise et factuelle." className={`${inputCls} resize-y`} />
        </div>

        <div>
          <label className={labelCls}>Causes fréquentes <span className="normal-case font-normal text-slate-500">(1 par ligne)</span></label>
          <textarea value={causes} onChange={(e) => setCauses(e.target.value)} rows={4}
            placeholder={"Cause 1\nCause 2\nCause 3"} className={`${inputCls} resize-y`} />
        </div>
        ...
        <div>
          ...
          <textarea value={redFlags} onChange={(e) => setRedFlags(e.target.value)} rows={4}
            placeholder={"Signe d'alerte 1\nSigne d'alerte 2"} className={`${inputCls} resize-y`} />
        </div>
        ...
        <div>
          ...
          <textarea value={whenToConsult} onChange={(e) => setWhenToConsult(e.target.value)} rows={3}
            className={`${inputCls} resize-y`} />
        </div>

        <div>
          <label className={labelCls}>FAQ <span className="normal-case font-normal text-slate-500">{`(JSON : [{ "q": "…", "a": "…" }])`}</span></label>
          <textarea value={faqJson} onChange={(e) => setFaqJson(e.target.value)} rows={6}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={relatedSlugs} onChange={(e) => setRelatedSlugs(e.target.value)}
              placeholder="mal-de-tete-causes, migraine-traitement" className={inputCls} />
          </div>
          <div>
            ...
            <input type="text" value={glossarySlugs} onChange={(e) => setGlossarySlugs(e.target.value)}
              placeholder="cephalee, migraine" className={inputCls} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-3">
        ...
        <textarea value={sources} onChange={(e) => setSources(e.target.value)} rows={5}
          placeholder={`[\n  { "label": "OMS — Céphalées", "url": "https://www.who.int/…", "publisher": "OMS", "year": "2024" }\n]`}
          className={`${inputCls} resize-y font-mono text-xs`} />
      </div>
        ...
        {arOpen && (
          <div className="px-6 pb-6 space-y-5 border-t border-slate-100 pt-5">
            ...
            <div>
              ...
              <input type="text" dir="rtl" value={termAr} onChange={(e) => setTermAr(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Réponse courte (AR)</label>
              <textarea dir="rtl" value={shortAnswerAr} onChange={(e) => setShortAnswerAr(e.target.value)} rows={3} className={`${inputCls} resize-y`} />
            </div>
            <div>
              <label className={labelCls}>Causes (AR) <span className="normal-case font-normal text-slate-500">(1 par ligne)</span></label>
              <textarea dir="rtl" value={causesAr} onChange={(e) => setCausesAr(e.target.value)} rows={4} className={`${inputCls} resize-y`} />
            </div>
            <div>
              <label className={labelCls}>Signes d’alerte (AR) <span className="normal-case font-normal text-slate-500">(1 par ligne)</span></label>
              <textarea dir="rtl" value={redFlagsAr} onChange={(e) => setRedFlagsAr(e.target.value)} rows={4} className={`${inputCls} resize-y`} />
            </div>
            <div>
              <label className={labelCls}>Quand consulter (AR)</label>
              <textarea dir="rtl" value={whenToConsultAr} onChange={(e) => setWhenToConsultAr(e.target.value)} rows={3} className={`${inputCls} resize-y`} />
            </div>
            <div>
              <label className={labelCls}>FAQ (AR) <span className="normal-case font-normal text-slate-500">(JSON [{`{ "q", "a" }`}])</span></label>
              <textarea dir="rtl" value={faqJsonAr} onChange={(e) => setFaqJsonAr(e.target.value)} rows={5} className={`${inputCls} resize-y font-mono text-xs`} />
            </div>
            <div>
              ...
              <textarea dir="rtl" value={sourcesAr} onChange={(e) => setSourcesAr(e.target.value)} rows={4} className={`${inputCls} resize-y font-mono text-xs`} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none">
          ...
          <span className="text-sm font-medium text-slate-700">
            Marquer comme relu (FR) aujourd’hui — autorise l’indexation
            {topic?.reviewedAt && <span className="text-xs text-slate-400 font-normal"> (déjà relu)</span>}
          </span>
        </label>
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none">
          <input type="checkbox" checked={markArReviewed} onChange={(e) => { setMarkArReviewed(e.target.checked); if (e.target.checked) setUnmarkArReviewed(false); }}
            className="w-4 h-4 rounded border-slate-300 text-secondary-600 focus:ring-secondary-500" />
          <span className="text-sm font-medium text-slate-700">
            Marquer comme relu (AR) aujourd’hui — autorise l’affichage arabe
            {topic?.arReviewedAt && <span className="text-xs text-slate-400 font-normal"> (déjà relu)</span>}
          </span>
        </label>
      ...
      <div className="flex items-center justify-between gap-4">
        <Link href="/admin/symptomes" className="text-sm text-slate-500 hover:text-slate-700 transition-colors">
          ← Retour à la liste
        </Link>
