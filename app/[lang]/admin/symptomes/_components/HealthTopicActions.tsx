"use client";
import { deleteTopic } from "@/features/health-topic/actions";

export function DeleteBtn({ id, term }: { id: string; term: string }) {
  return (
    <form
      action={deleteTopic.bind(null, id)}
      onSubmit={(e) => {
        if (!confirm(`Supprimer le symptôme "${term}" ? Cette action est irréversible.`)) {
          e.preventDefault();
        }
      }}
    >
