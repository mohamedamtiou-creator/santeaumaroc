...
import type { ExistingReview } from "./ReviewDialog";

/**
 * Formulaire d'avis — extrait de ReviewDialog et chargé en `dynamic(ssr:false)`
 * au 1ᵉʳ clic sur « Laisser un avis » (voir ReviewDialog). Défère le JS du
 * formulaire (useActionState, submitReview, sélecteur d'étoiles) hors du bundle
 * initial de la fiche praticien pour les ~99 % de visiteurs qui ne notent pas.
 * L'a11y de la modale (focus/escape/verrou de scroll) reste dans le launcher.
 */

type ReviewT = Dictionary["review"];

/* ── Couleurs des étoiles (les libellés viennent du dictionnaire) ── */
...
function StarSvg({ filled }: { filled: boolean }) {
  return (
    <svg
      ...
      className="w-9 h-9 sm:w-10 sm:h-10 transition-colors duration-100"
      aria-hidden="true"
    >
      <path
      ...
      <div className="flex items-center gap-0.5"
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            className={[
              "rounded-lg p-1 transition-transform duration-100",
...
/* ── Formulaire de l'avis (composant isolé pour réinitialiser useActionState) ── */
...
export function ReviewForm({ doctorId, doctorSlug, existingReview, onClose, onSuccess, t }: FormProps) {
  const [rating, setRating]   = useState(existingReview?.rating ?? 0);
  const [comment, setComment] = useState(existingReview?.comment ?? "");
  const [state, formAction, isPending] = useActionState(submitReview, undefined);
  const isEdit = !!existingReview;
  const toastFired = useRef(false);

  /* Succès : déclencher toast immédiatement, fermer la modale après 2 s */
  useEffect(() => {
  ...
  return (
    <form action={formAction} noValidate>
      ...
      <div className="px-5 sm:px-6 py-6 border-t border-slate-100">
        ...
        {state?.errors?.rating && (
          <p className="text-xs text-rose-500 text-center mt-2" role="alert">
      ...
      {globalError && (
        <div role="alert"
          className="mx-5 sm:mx-6 mb-4 px-4 py-3 rounded-xl bg-rose-50 border border-rose-100 text-sm text-rose-700 flex items-center gap-2.5">
      ...
      {/* ── Message de succès ── */}
      {success && (
        <div className="mx-5 sm:mx-6 mb-4 px-4 py-3 rounded-xl bg-secondary-50 border border-secondary-100 text-sm text-secondary-700 flex items-center gap-2.5">
          <CheckCircleIcon />
      ...
      <div className="px-5 sm:px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex flex-col-reverse sm:flex-row items-center gap-3 sm:justify-between">
        <p className="text-xs text-slate-500 leading-relaxed text-center sm:text-start">
          {t.guidelines}
        </p>
        <button
          type="submit"
          ...
          style={
            !isPending && rating > 0 && !success
              ? { boxShadow: "0 1px 3px 0 rgb(5 150 105 / 0.35)" }
              : undefined
          }
        >
          {isPending ? (
