...
import { isTopicArReady, isTopicReviewed, composeTreatmentQuestion } from "@/lib/health-topic";
...
const getTopics = () =>
  prisma.healthTopic.findMany({
    ...
    select: {
      ...
      treatmentSummary: true, treatmentSummaryAr: true,
...
/** Accroche : 1re phrase du résumé de traitement (localisé), tronquée. */
function teaser(tp: { treatmentSummary: string | null; treatmentSummaryAr: string | null; arReviewedAt: Date | null; shortAnswerAr: string | null }, locale: Locale): string {
  const ar = locale === "ar" && isTopicArReady(tp);
  const src = ((ar && tp.treatmentSummaryAr ? tp.treatmentSummaryAr : tp.treatmentSummary) ?? "").trim();
  if (!src) return "";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = getDictionary(locale).treat;
  return {
    title: t.metaTitle,
...
export default async function TreatmentIndexPage({ params }: { params: Promise<{ lang: string }> }) {
  const locale = toLocale((await params).lang);
  ...
  // Regroupe par spécialité (nom localisé), chaque lien porte la question composée.
  ...
  for (const tp of topics) {
    ...
    const entry = { slug: tp.slug, question: composeTreatmentQuestion(term, locale), teaser: teaser(tp, locale) };
    ...
    "@graph": [
      ...
      },
      {
        ...
        "itemListElement": reviewedFlat.map((tp, i) => ({
          "@type": "ListItem",
          "position": i + 1,
          "name": composeTreatmentQuestion(localizedTerm(tp, locale), locale),
    ...
    <>
      ...
      <div className="page-outer">
        <div className="max-w-3xl mx-auto">
          {/* Bloc éditorial : mode d'emploi + garde-fou YMYL */}
          <section aria-labelledby="howto-title" className="mb-8 rounded-2xl border border-amber-200 bg-amber-50/50 p-5 sm:p-6">
