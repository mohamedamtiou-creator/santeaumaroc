...
import { submitEstablishmentReview } from "@/features/reviews/actions";
import type { Dictionary } from "@/lib/i18n";
import type { EstabExistingReview } from "./EstablishmentReviewDialog";

/**
 * Formulaire d'avis établissement — extrait de EstablishmentReviewDialog et
 * chargé en `dynamic(ssr:false)` au 1ᵉʳ clic. Défère le JS du formulaire
 * (useActionState, submitEstablishmentReview, étoiles) hors du bundle initial
 * des fiches établissement. L'a11y de la modale (escape/scroll) reste au launcher.
 */
...
export function EstablishmentReviewForm({
  establishmentId, slug, basePath, initialRating, existingReview, onClose, onSuccess, t,
}: {
  ...
  const [state, formAction, isPending] = useActionState(submitEstablishmentReview, undefined);
  ...
  return (
    <form action={formAction} noValidate>
      ...
      <div className="px-5 sm:px-6 pb-5">
        <label htmlFor="estab-review-comment" className="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">
          {t.comment}{" "}
          <span className="font-normal text-slate-500 normal-case tracking-normal">{t.optional}</span>
        </label>
        <textarea
          id="estab-review-comment"
          name="comment"
          value={comment}
