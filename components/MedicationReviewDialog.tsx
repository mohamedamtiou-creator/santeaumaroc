...
export type MedExistingReview = { rating: number; comment: string | null } | null;
...
/** Base de route de la fiche médicament — pour le login (callbackUrl) + revalidation. */
const BASE_PATH = "/medicaments";
...
/* Formulaire lourd chargé au 1ᵉʳ clic (ssr:false) — hors bundle initial des
   fiches médicament. Le launcher (trigger + a11y escape/scroll) reste synchrone. */
const MedicationReviewForm = dynamic(
  () => import("./MedicationReviewForm").then((m) => m.MedicationReviewForm),
  {
    ssr: false,
    loading: () => (
...
export function MedicationReviewDialog({
  medicationId, slug, medicationName,
  ...
  // Session + avis existant : SORTIS du rendu serveur (la fiche est statique).
  ...
  const [fetchedReview, setFetchedReview] = useState<MedExistingReview>(null);
  ...
  useEffect(() => {
    ...
    let alive = true;
    fetch(`/api/medicaments/${medicationId}/my-review`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (alive && d) setFetchedReview(d.existingReview ?? null); })
      .catch(() => {});
    return () => { alive = false; };
  }, [isLoggedIn, medicationId]);

  const loginUrl = `/connexion?callbackUrl=${encodeURIComponent(`${BASE_PATH}/${slug}#avis`)}`;
  ...
  const handleSuccess = (wasEdit: boolean) => {
    ...
    // apparaissent immédiatement (revalidateTag/revalidatePath ont marqué le cache périmé).
    router.refresh();
  };
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4"
            role="dialog" aria-modal="true" aria-labelledby="med-review-dialog-title">
            <div className="bg-white w-full sm:max-w-md sm:rounded-2xl shadow-2xl overflow-hidden rounded-t-3xl">
              ...
              <div className="px-5 sm:px-6 pt-5 pb-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 id="med-review-dialog-title" className="text-base font-bold text-slate-900">
                    {isEdit ? t.titleEdit : t.titleNew}
                  </h2>
                  <p className="text-sm text-slate-500 mt-0.5 truncate">{medicationName}</p>
                </div>
